...
void *tree_sitter_lyra_external_scanner_create() {
...
unsigned tree_sitter_lyra_external_scanner_serialize(void *payload,
                                                     char *buffer) {
  Scanner *scanner = (Scanner *)payload;
...
void tree_sitter_lyra_external_scanner_deserialize(void *payload,
                                                   const char *buffer,
                                                   unsigned length) {
  Scanner *scanner = (Scanner *)payload;
  if (length == 0)
...
bool tree_sitter_lyra_external_scanner_scan(void *payload, TSLexer *lexer,
  ...
  if (valid_symbols[BLOCK_STRING_CONTENT] &&
      scan_block_content(scanner, lexer)) {
