/**
 * @file Lyra grammar for tree-sitter
 * @author Daniel Alanis
 * @license MIT
 */
...
export default grammar({
  ...
  rules: {
    ...
    lyra_command: ($) => choice($.lyra_simple_command, $.lyra_subshell),
    lyra_subshell: ($) => seq("(", $.lyra_list, ")"),
    lyra_simple_command: ($) =>
      prec.right(
        choice(
          seq(
            repeat($.lyra_cmd_prefix),
            $.lyra_cmd_word,
            repeat($.lyra_cmd_suffix),
          ),
          repeat1($.lyra_cmd_prefix),
        ),
      ),
    lyra_cmd_prefix: ($) => choice($.lyra_assignment, $.lyra_redirection),
    lyra_cmd_suffix: ($) => choice($.lyra_word, $.lyra_redirection),
    lyra_cmd_word: ($) => $.lyra_word,
    ...
    lyra_command_subst: ($) => seq("$(", $.non_empty_chunk, ")"),
    lyra_command_subst_immediate: ($) =>
      seq($.lyra_command_subst_start_immediate, $.non_empty_chunk, ")"),
    lyra_command_subst_start_immediate: (_) => token.immediate("$("),
    lyra_variable: ($) =>
      choice(seq("$", $.lyra_name), seq("${", $.lyra_param_expr, "}")),
    lyra_variable_immediate: ($) =>
      choice(
        seq(token.immediate("$"), $.lyra_name),
        seq(token.immediate("${"), $.lyra_param_expr, "}"),
      ),
    lyra_redirection: ($) =>
      seq(optional($.lyra_fd), $.lyra_redir_op, $.lyra_word),
    lyra_fd: ($) => $.lyra_digits,
    lyra_redir_op: (_) =>
      choice("<", ">", ">>", "<>", "<<", "<<-", ">&", "<&", ">|", ">&-", "<&-"),
